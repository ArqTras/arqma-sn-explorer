...
Current development branch:
 ...
 - https://github.com/doy-lee/onion-arq-blockchain-explorer/tree/arq
...
git clone --recursive https://github.com/arq-project/arq
...
```bash
# go to home folder if still in ~/arq
cd ~

# download the source code
git clone https://github.com/arq-project/arq-onion-blockchain-explorer.git

# enter the downloaded sourced code folder
cd arq-onion-blockchain-explorer
...
# alternatively can use: cmake -DARQMA_DIR=/path/to/arq ..
# if arq is not in ~/arq
#
...
By default it will look for blockchain in its default location i.e., `~/.arq/lmdb`.
...
```
arqblocks, Onion Loki Blockchain Explorer:
...
alias arqblocksmainnet='~/arq-onion-blockchain-explorer/build/arqblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias arqblockstestnet='~/arq-onion-blockchain-explorer/build/arqblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.arq/lmdb/emission_amount.txt`. For testnet or stagenet networks, 
 it is `~/.arq/testnet/lmdb/emission_amount.txt` or `~/.arq/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.arq/lmdb/emission_amount.txt`
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_arq",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "arq_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
