...
uint64_t const founders_locked_arq            = 1215000 * COIN;
uint64_t const seed_locked_arq                = 581000 * COIN;
uint64_t const half_seed_locked_arq           = seed_locked_arq * COIN / 2;
static std::map<BlockHeight, uint64_t> time_locked_arq =
{
    {0,      871500 * COIN},
    {69456,  founders_locked_arq + seed_locked_arq},
    {134079, founders_locked_arq + seed_locked_arq},
    {198639, founders_locked_arq + seed_locked_arq},
    {263423, founders_locked_arq + half_seed_locked_arq}
};
...
CurrentBlockchainStatus::calculate_emission_in_blocks(uint64_t start_blk, uint64_t end_blk, uint64_t &unlocked)
{
    ...
    for (;start_blk < end_blk; start_blk++)
    {
      ...
      if (start_blk == 0)
      {
        for (auto const &it : time_locked_arq)
            emission_calculated.circulating_supply -= it.second;
      }

      auto unlock_it = time_locked_arq.find(start_blk);
      if (unlock_it != time_locked_arq.end())
...
bf::path CurrentBlockchainStatus::blockchain_path {"/home/mwo/.arq/lmdb"};
