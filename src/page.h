...
struct tx_details
{
    ...
    get_mstch_map() const
    {
        ...
        const double arq_amount = LOK_AMOUNT(fee);
        ...
        if (!input_key_imgs.empty())
        {
            ...
            fee_micro_str    = fmt::format("{:04.0f}" , arq_amount * 1e6);
...
void add_tx_metadata(mstch::map &context, const cryptonote::transaction &tx, bool detailed = false) {
    ...
    if (tx.type == cryptonote::txtype::state_change)
    {
        ...
        if (new_style || get_service_node_state_change_from_tx_extra(tx.extra, state_change, cryptonote::network_version_11_infinite_staking)) {
           if (new_style) context["state_change_new_style"] = true;
            ...
            if (detailed) {
                ...
                // recent, this requires arqd to be started with --store-quorum-history (PR #702)
    ...
    else if (tx.version >= txversion::v4_tx_types && tx.type == txtype::arq_name_system)
    {
      tx_extra_arq_name_system lns_extra = {};
      if (get_arq_name_system_from_tx_extra(tx.extra, lns_extra))
      {
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    context["network_info"] = mstch::map {
            {"difficulty"         , xmreg::make_comma_sep_number(current_network_info.difficulty)},
            {"hash_rate"          , hash_rate},
    ...
    if (CurrentBlockchainStatus::is_thread_running())
    {
        ...
        std::string emission_supply   = xmreg::make_comma_sep_number(current_values.circulating_supply / COIN);
        std::string emission_coinbase = xmreg::make_comma_sep_number(current_values.coinbase / COIN);
        std::string emission_fee      = xmreg::make_comma_sep_number(current_values.fee / COIN);
        std::string emission_burn     = current_values.burn ? xmreg::make_comma_sep_number(current_values.burn / COIN) : "0";
...
show_block(uint64_t _blk_height)
{
    ...
    mstch::map context {
            ...
            {"blk_difficulty"       , xmreg::make_comma_sep_number(blk_difficulty)},
            ...
            {"blk_cumulative_difficulty" , xmreg::make_comma_sep_number(blk_cumulative_difficulty)},
    };
                ...
                bool tx_prove = false)
{
    ...
    // parse string representing given arq address
    ...
    if (!xmreg::parse_str_address(arq_address_str,  address_info, nettype))
    {
        ...
        return string("Cant parse Loki address: " + arq_address_str);
    }
...
//             << pod_to_hex(tx_pubkey) << '\n'  << '\n';
    ...
    // we get number of all posibilities, and their total arq amount
...
//                                << endl;
    ...
    // useful for unit testing as it provides total arq sum
    // of possible mixins
...
search(string search_text)
{
    ...
    // check if arq address is given based on its length
    ...
    // check if integrated arq address is given based on its length
...
json_networkinfo()
{
    ...
    if (!get_arq_network_info(j_info))
    {
        j_response["status"]  = "error";
...
json_version()
{
    ...
    j_data = json {
            ...
            {"arq_version_full"   , string {ARQMA_VERSION_FULL}},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    mstch::map context {
            ...
            {"tx_fee_short"          , xmreg::arq_amount_to_str(txd.fee, "{:0.4f}", false)},
            {"tx_fee_micro"          , xmreg::arq_amount_to_str(txd.fee*1e6, "{:0.4f}", false)},
    ...
    add_tx_metadata(context, tx, true /*detailed*/);
...
bool
get_arq_network_info(json& j_info)
{
