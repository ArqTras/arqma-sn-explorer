...
main(int ac, const char* av[])
{
    ...
    if (enable_emission_monitor == true)
    {
        ...
        // e.g., ~/.arq/lmdb/emission_amount.txt.
...
//        });
        ...
        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
        ([&](string tx_hash) {
            return myarq::jsonresponse {arqblocks.show_ringmemberstx_jsonhex(remove_bad_chars(tx_hash))};
        });
    ...
    ([&](const crow::request& req, std::string service_node_pubkey) {
        return crow::response(arqblocks.show_service_node(remove_bad_chars(service_node_pubkey)));
    });

    CROW_ROUTE(app, "/service_nodes")
    ([&](const crow::request& req) {
        return arqblocks.render_service_nodes_html(true /*add_header_and_footer*/);
    });

    CROW_ROUTE(app, "/quorums")
    ([&](const crow::request& req) {
        return arqblocks.render_quorum_states_html(true /*add_header_and_footer*/);
    });

    CROW_ROUTE(app, "/checkpoint_quorum/<uint>")
    ([&](const crow::request& req, uint64_t height) {
        return arqblocks.render_single_quorum_html(service_nodes::quorum_type::checkpointing, height);
    });

    CROW_ROUTE(app, "/obligations_quorum/<uint>")
    ([&](const crow::request& req, uint64_t height) {
        return arqblocks.render_single_quorum_html(service_nodes::quorum_type::obligations, height);
    });

    CROW_ROUTE(app, "/checkpoints")
    ([&](const crow::request& req) {
        return arqblocks.render_checkpoints_html(true /*add_header_and_footer*/);
    });

    // TODO(arq): This should be combined into the normal search mechanism, we shouldn't have 2 search bars.
    CROW_ROUTE(app, "/search_service_node").methods("GET"_method)
    ([&](const crow::request& req) {
        return arqblocks.show_service_node(remove_bad_chars(string(req.url_params.get("value"))));
    });
...
//    });
    ...
    if (enable_json_api)
    {
        ...
        ([&](string tx_hash) {
            ...
            myarq::jsonresponse r{arqblocks.json_detailedtransaction(remove_bad_chars(tx_hash))};
        ...
        CROW_ROUTE(app, "/api/circulating_supply") ([&]() {
            std::string result = std::to_string(xmreg::CurrentBlockchainStatus::get_emission().coinbase / COIN);
